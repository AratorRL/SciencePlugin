...
struct FQuat {
	...
	FQuat() : FQuat(1.0, 0.0, 0.0, 0.0) {}

	inline FQuat conjugate()
	{
		return FQuat(W, -X, -Y, -Z);
	}

	inline FQuat operator*(FQuat q2)
	{
		float mulW = W * q2.W;
		mulW -= X * q2.X;
		mulW -= Y * q2.Y;
		...
		return FQuat(mulW, mulX, mulY, mulZ);
	}
};
...
struct Vector2 {
	...
	inline Vector2 rotate(float theta) {
